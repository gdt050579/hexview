...
impl ViewWin {
    pub fn new(path: &Path) -> Self {
        let mut win = ViewWin {
            ...
            endian: bufferview::Endian::Little,
        };
        ...
        win.endian_little = endian_menu.add(menu::SingleChoice::new("&Little", Key::None, viewwin::Commands::EndianLittle, true));
        win.endian_big = endian_menu.add(menu::SingleChoice::new("&Big", Key::None, viewwin::Commands::EndianBig, false));
    ...
    fn endian_label(&self) -> &'static str {
        match self.endian {
            bufferview::Endian::Little => "Endian:Little",
            bufferview::Endian::Big => "Endian:Big",
        }
    }

    fn set_endian(&mut self, endian: bufferview::Endian) {
        self.endian = endian;
        let h = self.bv;
        if let Some(bv) = self.control_mut(h) {
    ...
    fn toggle_endian(&mut self) {
        let next = match self.endian {
            bufferview::Endian::Little => bufferview::Endian::Big,
            bufferview::Endian::Big => bufferview::Endian::Little,
        };
...
impl MenuEvents for ViewWin {
    fn on_menu_open(&self, menu: &mut Menu) {
        ...
        let h_endian = match self.endian {
            bufferview::Endian::Little => self.endian_little,
            bufferview::Endian::Big => self.endian_big,
        };
    ...
    fn on_select(&mut self, _menu: Handle<Menu>, _item: Handle<menu::SingleChoice>, command: viewwin::Commands) {
        match command {
            ...
            viewwin::Commands::EndianLittle => self.set_endian(bufferview::Endian::Little),
            viewwin::Commands::EndianBig => self.set_endian(bufferview::Endian::Big),
            _ => {}
        }
    }
}
